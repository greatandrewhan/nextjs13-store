  ...
  return ReactDom.createPortal(
    <div className="fixed top-0 left-0 w-screen h-screen z-50">
      ...
      <div className="flex flex-col bg-white absolute right-0 top-0 h-screen shadow-lg w-screen sm:w-96 max-w-screen gap-4">
        ...
        <div className="p-4 overflow-scroll overflow-x-hidden overflow-y-hidden flex-1 flex flex-col gap-4">
          {cartItems.length === 0 ? (
            ...
            <>
              {cartItems.map((cartItem, itemIndex) => {
                return (
                  <div
                    key={itemIndex}
                    className="flex border-l border-solid border-slate-700 p-2 flex-col gap-2"
                  >
        ...
        <div
          onClick={checkout}
          className="border border-solid border-slate-700 text-xl m-4 p-6 uppercase grid place-items-center cursor-pointer"
        >
          Checkout
        </div>
      </div>
